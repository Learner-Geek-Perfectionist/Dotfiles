...
2. **[K-002]** `smart_close.py` 调用不存在的 `boss.window` — 运行时必崩 AttributeError
3. **[H-006]** `systemInfo.lua` 复制粘贴 bug `obj.last_up` 丢失 — 无网络时运行时崩溃
4. **[I-008/S-023]** install.sh 与 lib/utils.sh 重复 85 行 + 跨脚本 100+ 行下载模式重复 — 最大维护风险
5. **[Z-002]** age-tokens trap 引号问题 + 临时文件未清理 — 明文 token 泄漏风险
