...
if [[ $(uname -s) == "Darwin" ]]; then
	...
	if [[ $os_type == "ubuntu" ]]; then
		sudo apt update
		tools=("zsh" "libzstd-dev" "git" "curl" "make" "g++" "gcc" "openssh-server" "mandb" "wget" "gnupg" "pkg-config" "xz-utils" "openssh-server" "gtk-update-icon-cache" "libzstd-dev" "libssl-dev")
		# 遍历工具列表，检查是否已安装
		for tool in "${tools[@]}"; do
			if ! command -v "$tool" >/dev/null 2>&1; then
