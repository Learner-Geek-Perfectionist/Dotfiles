...
qk.apps = {
    ...
    audioMIDISetup = 'com.apple.audio.AudioMIDISetup',
    baiduNetdisk = 'com.baidu.BaiduNetdisk-mac',
    camera = 'com.apple.PhotoBooth',
    chatgpt = 'com.openai.chat',
    chrome = 'com.google.Chrome',
    calendar = 'com.apple.iCal',
    cotEditor = 'com.coteditor.CotEditor',
    datagrip = 'com.jetbrains.datagrip',
    enchanted = 'subj.Enchanted',
    finder = 'com.apple.finder',
    ghostty = 'com.mitchellh.ghostty',
}
selectMenuItem = function(...)
    -- ⚠️ 不要传入字符串 搜索非常慢!!!
    hs.application.frontmostApplication():selectMenuItem(...)
end
...
function mod.superOpenApp(appID)
    ...
    if (appID ~= currApp:bundleID()) then
        ...
        if (appID == qk.apps.finder) then
            local wins = _filter(app:allWindows(), function(x)
                return x:title() ~= ''
            end)
            if (#wins > 0) then
                wins[1]:focus()
        ...
        local currWins = _filter(currApp:allWindows(), function(x)
            return x:isStandard()
        end)
        -- 如果没有标准窗口, 则什么都不做(
        if (#currWins == 0) then
