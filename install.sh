...
prompt_download_fonts() {
    ...
    echo "\n在虚拟机中运行时，字体渲染依赖虚拟机特定的字体，因此需要安装字体。\n"
    echo "\n在 Docker 容器(或 WSL)中运行时，通常不需要在容器(或WSL)内安装字体，但应确保宿主机已安装适当的字体以支持任何可能的字体渲染需求。\n"
    echo "\n宿主机一般需要良好的字体支持来确保所有应用和终端模拟器都能正常渲染字符。\n"
    read -p "\n是否需要下载字体以支持终端模拟器的渲染？(y/n): " download_confirm < /dev/tty
