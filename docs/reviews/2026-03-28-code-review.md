...
1. **[C-001]** `.ssh/config` 全局 `ForwardAgent yes` — 安全漏洞，不受信任的远程主机可冒充你登录其他系统
2. ~~**[K-002]** `smart_close.py` 调用不存在的 `boss.window` — 运行时必崩 AttributeError~~ **已修复**
3. ~~**[H-006]** `systemInfo.lua` 复制粘贴 bug `obj.last_up` 丢失 — 无网络时运行时崩溃~~ **已修复**
4. ~~**[I-008/S-023]** install.sh 与 lib/utils.sh 重复 85 行 + 跨脚本 100+ 行下载模式重复 — 最大维护风险~~ **已修复**
5. ~~**[Z-002]** age-tokens trap 引号问题 + 临时文件未清理 — 明文 token 泄漏风险~~ **已修复**

## 问题列表

### Critical

#### [I-008] ~~install.sh 与 lib/utils.sh 存在 ~85 行重复代码~~ **已修复**
...
- **修复:** 将 93 行简化为 51 行 bootstrap 版本，去掉 `_display_width` 和复杂的 `print_banner`（改为简化版），clone 后由 `lib/utils.sh` 完整版覆盖。

---

#### [K-001] ~~`cmd+0` 快捷键映射冲突，`goto_tab 10` 被静默覆盖~~ **已修复**
...
#### [K-002] ~~`smart_close.py` 中 `boss.window` 属性不存在，运行时 AttributeError~~ **已修复**
...
- **修复:** 重构为先判断 `len(tab.windows) == 1` 再决策：最后一个窗口用 `boss.close_os_window()`，否则 `window.close()`。同时修复了 K-003（竞态）、K-004（None 检查）、K-013（冗余逻辑）。

---

#### [H-006] ~~`systemInfo.lua` 中 `obj.last_down` 赋值两次，`obj.last_up` 丢失~~ **已修复**
...
- **修复:** 第 165 行 `obj.last_down = 0` → `obj.last_up = 0`。
...
#### [Z-002] ~~`edit-tokens` trap 引号嵌套 + 第二个临时文件未清理~~ **已修复**
...
- **修复:** trap 改用 `${(q)var}` 安全引用；`tmp_age` 创建后立即更新 trap 同时清理两个文件；`umask 077` 消除 TOCTOU 窗口；编辑器退出码改用 `if !` 直接检查（同时修复 Z-003、Z-026）。

---

#### [S-023] ~~跨脚本重复："下载-解压-替换-清理" 模式出现 5 次（100+ 行）~~ **已修复**
...
- **修复:** 在 `lib/utils.sh` 中添加 `download_and_extract()` 函数（支持 tar.gz/zip/gz 三种格式），重构 5 个安装函数调用它，消除 100+ 行重复。
...
#### [L-001] ~~缺少 set -euo pipefail 防护，与 packages.sh 不一致~~ **已修复**
- **文件:** `lib/utils.sh:1-3`
- **维度:** 代码质量
...
- **修复:** 在 `utils.sh` 头部添加注释说明不设 `set -e` 的原因和 return 0/1 约定；`packages.sh` 移除 `set -eo pipefail`（L-012）。两个库文件现在行为一致。
...
#### [L-012] ~~`set -eo pipefail` 在被 source 的纯数据文件中会影响调用方~~ **已修复**
...
- **修复:** 移除 `set -eo pipefail`，替换为注释说明由调用方负责。

---

#### [L-013] ~~`rustup` 和 `rust` 同时列出，可能冲突~~ **已修复**
...
- **修复:** 移除 `rust`，保留 `rustup` 管理工具链。

---

#### [I-001] ~~`sudo` 命令展开未加引号，依赖 word splitting~~ **已修复**
- **文件:** `install.sh:208`
- **维度:** 安全性
...
- **修复:** 用 `read -ra install_cmd` 转为数组，`sudo "${install_cmd[@]}"` 安全展开。

---

#### [I-004] ~~`rm_path` 对危险路径的保护不足~~ **已修复**
- **文件:** `uninstall.sh:57-58`
- **维度:** 安全性
- **描述:** `rm_path` 仅检查 `"$p" == "/"`，但 `//`、`/*`、`/tmp/../` 等变体同样危险。当前调用点全部使用硬编码路径，实际风险低，但作为通用函数防御不够。
- **修复建议:** 增加 `realpath` 规范化并限制只删除 `$HOME` 下的路径。
- **修复:** 添加 `realpath -m` 规范化路径，限制只允许删除 `$HOME` 或 `/tmp` 下的路径，其他路径拒绝并打印警告。

---

#### [I-005] ~~`uninstall.sh` 中 `sudo pmset` 未检查 `has_sudo`~~ **已修复**
...
- **修复:** 在 `pmset -g` 前添加 `has_sudo &&` 前置检查，无权限时跳过。
...
#### [I-009] ~~source `lib/utils.sh` 后 bootstrap 函数被覆盖，日志格式不一致~~ **已修复**
...
- **修复:** 在 source 处添加注释说明 bootstrap → 完整版的覆盖行为是预期设计。

---

#### [I-012] ~~install: Homebrew 包安装 / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** Homebrew 包可能被其他软件依赖，不自动卸载。添加提示信息引导用户参考 `lib/packages.sh` 手动卸载。

---

#### [I-013] ~~install: Kotlin/Native 安装 / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** 在 `remove_claude` 中添加 Kotlin/Native 目录和 bin 文件的清理。

---

#### [I-014] ~~install: npm LSP servers / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** 在 `remove_claude` 中添加 npm 全局包卸载、gopls 清理、csharp-ls 卸载（同时修复 I-015、I-017）。

---

#### [I-016] ~~install: Claude Marketplace + Plugins / uninstall: 仅清字段未实际卸载~~ **已修复**
...
- **修复:** 在 `remove_claude` 中添加遍历 `claude plugin list` 逐个卸载插件，并遍历 `claude plugin marketplace list` 移除 Marketplace。

---

#### [I-021] ~~install: pmset 设置两个参数 / uninstall: 仅恢复一个~~ **已修复**
...
- **修复:** 在 `sleep 1` 恢复后添加 `sudo pmset -a tcpkeepalive 0`。

---

#### [I-029] ~~`install_linux` 和 `install_macos` 存在重复的模式分发逻辑~~ **已修复**
...
- **修复:** 将 3 个 `*_ONLY` 检查从两个平台函数提升到 `main` 的 `case` 之前，消除 18 行重复。
...
#### [S-005] ~~`sudo xcode-select --reset` 无条件执行，可能覆盖用户设置~~ **已修复**
...
- **修复:** 改为 `xcode-select -p &>/dev/null || sudo xcode-select --reset`，仅在路径无效时重置。

---

#### [S-007] ~~VSCode 插件安装循环未捕获每个插件的退出码~~ **已修复**
...
- **修复:** 用 `|| install_rc=$?` 捕获每个安装命令的退出码，非零立即加入 `failed`。事后验证跳过已标记失败的项，避免重复计数。

---

#### [S-009] ~~pixi shell 集成会修改已由 Dotfiles 管理的 .zshrc~~ **已修复**
...
- **修复:** 简化 `install_pixi.sh`：去掉 `--install-only`/`--tools-only`/`--shell-only` 三个参数，只保留 full 模式一步完成（pixi + shell + 工具包）。`install.sh` 中合并 `install_pixi_binary` 和 `sync_pixi_tools` 为一次 `install_pixi_full` 调用，Linux 安装步骤从 6 步减为 5 步。

---

#### [S-011] ~~Claude 插件检测依赖脆弱的 CLI 输出格式~~ **已修复**
...
- **修复:** 改为 `grep -qF "$plugin"`，固定字符串匹配，不依赖缩进和 Unicode 格式。

---

#### [S-013] ~~jq 失败时空文件覆盖 settings.json，导致配置丢失~~ **已修复**
...
- **修复:** 所有 3 处 jq → .tmp → mv 模式均添加 `[[ -s "$settings_file.tmp" ]]` 非空检查，失败时 `rm -f .tmp` 并打印警告。

---

#### [S-017] ~~pixi `--manifest-path` 传递了目录而非文件~~ **已修复**
...
- **修复:** 所有 3 处 `--manifest-path` 及提示信息均改为 `"$HOME/pixi.toml"`。

---

#### [S-025] ~~jdtls wrapper 脚本 37 行 heredoc 内嵌函数中~~ **已修复**
...
- **修复:** 提取为 `scripts/wrappers/jdtls` 独立文件，安装时 `cp` 复制。函数从 37 行 heredoc 缩减为 6 行。

---

#### [S-029] ~~`setup_claude_hud` 函数 76 行，职责过多~~ **已修复**
...
- **修复:** 拆分为 3 个子函数，`setup_claude_hud` 主体缩减为 6 行调度逻辑。

---

#### [S-031] ~~Claude settings.json 合并逻辑 3 层回退过于复杂~~ **已修复**
...
- **修复:** 提取为 `_deploy_claude_settings()` 函数，main 中只需一行调用。逻辑不变，可读性提升。

---

#### [S-035] ~~VSCode 编辑器检测逻辑存在 code/cursor 重叠边界情况~~ **已修复**
- **文件:** `scripts/install_vscode_ext.sh:126-136`
- **维度:** 代码质量
- **描述:** 如果系统同时有 `code`（实为 cursor 别名）和真正的 `cursor` 命令，第二个会被跳过。
- **修复建议:** 基于二进制路径去重而非类型名。
- **修复:** 用 `realpath` 解析二进制路径，关联数组 `_seen_paths` 去重。同一个二进制只注册一次。

---

#### [Z-001] ~~`fzf-preview.sh` 缺少可执行权限~~ **已修复**
...
- **修复:** 先捕获 age 输出到变量，失败时打印黄色警告到 stderr，成功时 `eval` 加载。

---

#### [Z-005] ~~fzf 包装函数中 trap 会覆盖调用方已有的 trap~~ **已修复**
- **文件:** `.zshrc:164-169`
- **维度:** 代码质量
- **描述:** 在函数（非子 shell）中设置 EXIT/INT/TERM trap 会替换整个 shell 会话的 trap。
- **修复建议:** 使用 zsh 的 `always` 块替代 trap。
- **修复:** 改用 zsh `{ ... } always { rm -f "$tmp" }` 块，不影响 shell 全局 trap。

---

#### [Z-006] ~~`cat` 包装函数的逻辑可读性差~~ **已修复**
- **文件:** `.zshrc:185-192`
- **维度:** 代码质量
- **描述:** `||` 和 `&&` 优先级导致逻辑正确但难以阅读。
- **修复建议:** 拆分为显式的 `if` 分支。
- **修复:** 将 `[[ ]] || grep && { }` 链拆为两个独立的 `if` 分支，逻辑不变，可读性提升。
...
#### [Z-012] ~~`KEYTIMEOUT=1` 在 SSH 场景下可能破坏键位序列~~ **已修复**
...
- **修复:** 检测 `$SSH_CONNECTION`：SSH 中设为 5（50ms），本地保持 1（10ms）。50ms 足够覆盖大部分网络延迟，同时 double-esc 体感不会变慢。

---

#### [Z-013] ~~`double-esc-clear.zsh` 中文件描述符泄漏风险~~ **已修复**
...
- **修复:** 将 `zle -F`、`exec {fd}<&-`、`unset` 全部放入 `if [[ -n "$_esc_timer_fd" ]]` 保护块中，按正确顺序执行：先移除 fd watcher → 再关闭 fd → 最后 unset 变量。

---

#### [Z-015] ~~`--tac` 在 FZF_DEFAULT_OPTS 中影响所有 fzf 场景~~ **已修复**
- **文件:** `.zshrc:141`
- **维度:** 代码质量
- **描述:** 全局 `--tac` 对文件列表、补全候选等场景导致反直觉排序。
- **修复建议:** 移到 `FZF_CTRL_R_OPTS` 中只影响历史搜索。
- **修复:** 从 `FZF_DEFAULT_OPTS` 移除 `--tac`，新增 `FZF_CTRL_R_OPTS='--tac'` 仅影响 Ctrl+R。

---

#### [Z-018] ~~`reload` 别名重复加载导致环境污染~~ **已修复**
- **文件:** `.zshrc:207`
- **维度:** 代码质量
- **描述:** 重新 source 所有配置会累积 trap、zle widget、zinit 重复注册等全局状态。
- **修复建议:** 改为 `alias reload='exec zsh -l'`。
- **修复:** 改为 `exec zsh -l`，用全新的登录 shell 进程替换当前进程，环境干净无累积。
...
#### [Z-020] ~~`upgrade` 用 beta 分支，`uninstall` 用 master 分支~~ **已修复**
- **文件:** `.zshrc:210-211`
- **维度:** 代码质量
- **描述:** 两个别名使用不同分支，卸载脚本可能无法正确清理 beta 新增组件。
- **修复建议:** 统一使用相同分支。
- **修复:** `uninstall` 别名从 `master` 改为 `beta`，与 `upgrade` 统一。

---

#### [Z-026] ~~`edit-tokens` 明文临时文件的 TOCTOU 安全间隙~~ **已修复**
...
#### [K-003] ~~`smart_close.py` close 后立即检查 `tab.windows` 存在竞态~~ **已修复**
...
- **修复:** 见 K-002，重构为先判断再决策。

---

#### [K-004] ~~`smart_close.py` 未检查 `boss.active_window` 是否为 None~~ **已修复**
...
- **修复:** 见 K-002，第 17-18 行添加了 None 检查。

---

#### [K-005] ~~SSH 解析函数 `_extract_ssh_destination` 在两个文件中完全重复（35 行）~~ **已修复**
...
- **修复:** 提取 `extract_ssh_destination()` 和 `smart_launch()` 到 `ssh_utils.py`。两个脚本各从 ~80 行缩减为 ~22 行，只需一行 `smart_launch(boss, "tab"/"os-window")` 调用。同时修复 K-006（handle_result 重复）。

---

#### [K-013] ~~`smart_close.py` else 分支逻辑冗余且部分无效~~ **已修复**
...
- **修复:** 见 K-002，已重构为先判断再决策的简洁版本。

---

#### [K-014] ~~`listen_on unix:/tmp/kitty-socket` 多实例会冲突~~ **已修复**
- **文件:** `.config/kitty/kitty.conf:7`
- **维度:** 安全性
- **描述:** 固定 socket 路径，多个 Kitty 实例会冲突。
- **修复建议:** 使用 `listen_on unix:/tmp/kitty-{kitty_pid}`。
- **修复:** 改为 `listen_on unix:/tmp/kitty-{kitty_pid}`，每个实例独立 socket。

---

#### [H-001] ~~全局变量污染 — `keyConfig`/`HyperKey`/`windowsConfig` 未声明 local~~ **已修复**
- **文件:** `.hammerspoon/config/keyConfig.lua:3,23,25`
- **维度:** 代码质量
- **描述:** 三个配置变量均为全局变量，任何模块可意外覆盖。
- **修复建议:** 声明为 `local`，通过 `return` 导出模块表。
- **修复:** 改为 `local M = {}` + `return M` 模块返回模式。

---

#### [H-002] ~~全局变量污染 — `base.lua` 所有 11 个函数均为全局函数~~ **已修复**
...
- **修复:** 仅保留 `split` 和 `trim`（实际使用的），其余 9 个未使用函数全部删除。改为 `local M = {} ... return M`。同时修复 H-028（死代码删除）。

---

#### [H-003] ~~`unserialize` 使用 `load()` 执行任意字符串~~ **已修复**
...
- **修复:** 见 H-002，`unserialize` 和 `serialize` 均已删除。

---

#### [H-004] ~~命令注入 — `AppToggler` 拼接 bundleID 到 shell 命令~~ **已修复**
...
- **修复:** 改用原生 API `hs.application.launchOrFocusByBundleID`，完全避免 shell 调用。同时简化逻辑为 4 行（H-005）。

---

#### [H-007] ~~`systemInfo.lua` 网络接口仅加载时获取一次，切换后失效~~ **已修复**
- **文件:** `.hammerspoon/modules/systemInfo.lua:8`
- **维度:** 代码质量
- **描述:** Wi-Fi 切有线或断网重连后，`interface` 变量仍保存旧值。
- **修复建议:** 在 `init()` 或 `scan()` 中重新获取接口。
- **修复:** `interface` 初始化为 nil，在 `init()` 开头每次重新获取 `hs.network.primaryInterfaces()`。

---

#### [H-010] ~~`setSysInfo` 定时器 nil 检查可能空指针~~ **已修复**
...
- **修复:** 提取 `local INTERVAL = 3` 常量，所有分支统一 `bytes / INTERVAL / unit` 计算，定时器也引用 `INTERVAL`。

---

#### [H-013] ~~canvas 对象每 3 秒创建但从不 delete，资源泄漏~~ **已修复**
...
- **修复:** 取消注释 `canvas:delete()` 和 `canvas = nil`。

---

#### [H-016] ~~`reload.lua` 中 `configWatcher` 为全局变量~~ **已修复**
- **文件:** `.hammerspoon/modules/reload.lua:17`
- **维度:** 代码质量
- **描述:** 全局变量泄漏，应为 local。
- **修复建议:** 改为 `local configWatcher`。
- **修复:** 改为 local，通过 `return { _watcher = configWatcher }` 保持引用防止 GC。

---

#### [H-017] ~~`reload.lua` 中 `doReload` 为隐式全局变量~~ **已修复**
- **文件:** `.hammerspoon/modules/reload.lua:5`
- **维度:** 代码质量
- **描述:** 缺少 `local` 声明。
- **修复建议:** 改为 `local doReload = false`。
- **修复:** 已改为 `local doReload = false`。

---

#### [H-018] ~~`caffeinate.lua` 全局变量和函数~~ **已修复**
- **文件:** `.hammerspoon/modules/caffeinate.lua:2`
- **维度:** 代码质量
- **描述:** `caffeinateMode` 和 `toggleCaffeinateMode` 均为全局。
- **修复建议:** 改为 local，通过模块返回表导出。
- **修复:** 改为 `local M = {}` + `M.toggle()` + `return M`。

---

#### [H-020] ~~`windowManagement.lua` 全局函数 + 多余 require~~ **已修复**
...
- **修复:** 移除 `require "hs.window"`，改为 `local M = {}` + `M.moveToPosition()` / `M.switchFocusedAppWindow()` + `return M`。

---

#### [H-022] ~~`systemInfo.lua` 隐式依赖 `base.lua` 全局 `split` 函数~~ **已修复**
...
- **修复:** 改为 `local base = require('modules.base')`，调用 `base.split()`。

---

#### [H-023] ~~所有模块均无 `return` 语句，违反 Lua 模块规范~~ **已修复**
- **文件:** 所有 `.hammerspoon/modules/*.lua` 和 `config/*.lua`
- **维度:** 架构健康度
- **描述:** 全部依赖全局变量传递接口，无法单元测试，依赖关系不透明。
- **修复建议:** 统一采用 `local M = {} ... return M` 模块返回模式。
- **修复:** base、AppToggler、caffeinate、windowManagement、keyConfig 全部改为 `local M = {} ... return M`。reload 通过返回表保持 watcher 引用。

---

#### [H-024] ~~`KeyBinds.lua` 隐式依赖加载顺序~~ **已修复**
...
- **修复:** `KeyBinds.lua` 顶部显式 require `keyConfig`、`AppToggler`、`windowManagement`、`caffeinate`，不再依赖加载顺序。`init.lua` 简化为只 require `reload`、`systemInfo`、`KeyBinds`。

---

#### [E-001] ~~Library Code settings.json 含 Linux 特定值（环境残留）~~ **已修复**
...
- **修复:** 从两个 Library 版本中删除 `intelliSenseMode`、`compilerPath`、`compileCommands` 三项 Linux 残留。

---

#### [E-002] ~~Library Cursor settings.json 同样含 Linux 特定值~~ **已修复**
...
#### [E-004] ~~Cursor 缺少 autocorrect 远程扩展~~ **已修复**
- **文件:** `.config/Cursor/User/settings.json:128-129`
- **维度:** 架构健康度
- **描述:** Code 的 `remote.SSH.defaultExtensions` 含 `huacnlee.autocorrect`，但 Cursor 两个版本均缺少。autocorrect 通过 VSIX 手动安装到 Cursor（`CURSOR_VSIX` 数组），本地可用但远程 SSH 时不会自动安装。
- **修复建议:** 在 Cursor 的 `remote.SSH.defaultExtensions` 中添加 `"huacnlee.autocorrect"`。
- **修复:** Open VSX 不提供此扩展，无法通过 defaultExtensions 远程安装。在 Cursor 配置中添加注释说明原因。

---

#### [E-005] ~~file-deps.json 缺少 Library 路径的依赖映射~~ **已修复**
...
- **修复:** 添加 Library 下 8 个文件的双向依赖映射（.config ↔ Library，Code ↔ Cursor）。

---

#### [E-006] ~~sync-dotfile.sh 不处理编辑器配置路径~~ **已修复**
...
- **修复:** 添加 `.config/Code/*`、`.config/Cursor/*`、`Library/Application Support/*` 三个 case 分支，编辑任一版本自动同步到 ~/.config、Library（macOS）和对端编辑器。
...
#### [T-001] ~~get-my-ip.sh curl 调用缺少超时设置~~ **已修复**
- **文件:** `sh-script/get-my-ip.sh:8-9`
- **维度:** 代码质量
- **描述:** 两处 curl 无 `--connect-timeout` 和 `--max-time`，网络不通时脚本无限期挂起。
- **修复建议:** 添加 `--connect-timeout 5 --max-time 10`。
- **修复:** 两处 curl 均添加 `--connect-timeout 5 --max-time 10`。

---

#### [T-002] ~~get-my-ip.sh API Token 通过 URL 参数泄露~~ **已修复**
...
- **修复:** 改为 `curl -H "Authorization: Bearer ${API_TOKEN}" "https://ipinfo.io/${MY_IP}"`，token 不再出现在 URL 中。

---

#### [T-003] ~~get-my-ip.sh 获取公网 IP 失败时未检查直接传给下个 API~~ **已修复**
- **文件:** `sh-script/get-my-ip.sh:8`
- **维度:** 代码质量
- **描述:** 第一次 curl 失败时 `MY_IP` 为空，仍会用空值调用 ipinfo API。
- **修复建议:** 获取后立即验证 IP 格式。
- **修复:** 获取后用正则 `^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$` 校验，不合法立即退出。

---

#### [T-006] ~~get-my-ip.sh 硬编码网络接口名 en0/eth0~~ **已修复**
...
- **修复:** macOS 改为 `ipconfig getifaddr en0 || ipconfig getifaddr en1`，Linux 改为 `hostname -I | awk '{print $1}'`。

---

#### [D-001] ~~sync-dotfile.sh 变量未引号 + 缺少 `set -u`~~ **已修复**
...
- **修复:** `set -eo` → `set -euo`，`$REPO` 加引号 `"$REPO"/`。

---

#### [D-002] ~~sync-dotfile.sh 和 check-file-deps.sh stdin 读取方式不一致~~ **已修复**
- **文件:** `.claude/hooks/sync-dotfile.sh:7` vs `.claude/hooks/check-file-deps.sh:12-15`
- **维度:** 代码质量
- **描述:** 前者直接从 stdin 读取 jq，后者先 `cat` 缓存再 jq。风格不一致。
- **修复建议:** 统一为先缓存再提取的方式。
- **修复:** 改为 `input="$(cat)"` + `echo "$input" | jq ...`，与 check-file-deps.sh 风格统一。
...
#### [L-009] ~~`print_divider` 中 `width` 无默认值保护~~ **已修复**
- **文件:** `lib/utils.sh:172`
- **维度:** 代码质量
...
#### [L-010] ~~`get_local_version` 可用 bash 内置命令避免 fork~~ **已修复**
...
- **修复:** `cat` 替换为 `<"$version_file" read -r version && echo "$version"`。
...
#### [L-014] ~~数组元素 `Eudic` 大小写不一致~~ **已修复**
- **文件:** `lib/packages.sh:65`
- **维度:** 代码质量
- **描述:** 所有 cask 名称都用小写，唯独 `Eudic` 大写开头。Homebrew cask token 通常全小写。
- **修复建议:** 确认 Homebrew 中准确 token，统一为 `eudic`。
- **修复:** 已确认 Homebrew 正式 token 为 `eudic`（全小写），已统一。

---

#### [L-015] ~~`anaconda` cask 会修改 `.zshrc`，与 dotfiles 管理冲突~~ **不适用**
- **文件:** `lib/packages.sh:64`
- **维度:** 代码质量
- **描述:** `anaconda` 安装后会注入 `conda init` 代码块到 `.zshrc`，与仓库管理 `.zshrc` 的理念冲突。
- **修复建议:** 在注释中说明 conda 与 dotfiles 的共存策略。
- **关闭原因:** 未使用 `conda init`，而是在 `.zshrc:53` 手动将 `/opt/homebrew/anaconda3/bin` 加入 PATH，不存在注入冲突。

---

#### [L-016] ~~`cmake` 同时以 formula 和 cask 两种形式存在~~ **已修复**
...
- **修复:** 保留 cask `cmake-app`（CLI + GUI），移除 formula `cmake` + `cmake-docs`（消除重复二进制）。

---

#### [L-017] ~~packages.sh 作为纯数据文件，shebang 行冗余~~ **已修复**
...
- **修复:** shebang 保留（编辑器识别），`set -eo pipefail` 已在 L-012 中移除。

---

#### [I-002] ~~`print_banner` 中 `width` 可能为空（install.sh 副本）~~ **已修复**
- **文件:** `install.sh:118`
- **维度:** 代码质量
- **描述:** 与 L-006 相同问题在 install.sh 副本中重现。三种宽度检测全部失败时 `width` 为空。
- **修复建议:** 同 L-006，添加默认值 `width=80`。
- **修复:** 见 I-008 bootstrap 重构，已有 `width=80` 兜底。

---

#### [I-003] ~~`${#right_pad} -lt 0` 永假死代码（install.sh 副本）~~ **已修复**
- **文件:** `install.sh:122`
- **维度:** 代码质量
- **描述:** 与 L-007 相同问题在 install.sh 副本中重现。
- **修复建议:** 同 L-007。
- **修复:** 见 I-008 bootstrap 重构，改为 `right_width` 变量检查。

---

#### [I-007] ~~`install.sh` 缺少 `set -u`~~ **已修复**
- **文件:** `install.sh:9`
- **维度:** 代码质量
- **描述:** 使用 `set -eo pipefail` 但未开启 `-u`，引用未定义变量不报错。
- **修复建议:** 考虑改为 `set -euo pipefail`，对允许为空的变量使用 `${VAR:-}`。
- **修复:** 改为 `set -euo pipefail`，所有变量已有 `${VAR:-default}` 保护。

---

#### [I-015] ~~install: csharp-ls (dotnet tool) / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** 见 I-014，在 `remove_claude` 中统一添加。

---

#### [I-017] ~~install: gopls / uninstall: 缺少卸载~~ **已修复**
...
#### [I-018] ~~install: ripgrep config / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** `.config/` 循环从 `{zsh,kitty}` 扩展为 `{zsh,kitty,ripgrep}`。

---

#### [I-022] ~~install: access_bpf 组 / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** 在 macOS 分支中添加 `dseditgroup -d` 移除用户，带 `has_sudo` 前置检查。

---

#### [I-023] ~~install: Linux keychain / uninstall: 缺少卸载~~ **已修复**
...
- **修复:** 在 `remove_dotfiles` 中添加 `rm_path ~/.local/bin/keychain`。
...
#### [S-003] ~~MCP Server 配置硬编码本地代理地址~~ **已修复**
...
- **修复:** 提取 `detect_editor_type()` 到 `lib/utils.sh`，`install_dotfiles.sh` 的 `has_vscode`/`has_cursor` 和 `install_vscode_ext.sh` 统一使用。

---

#### [S-016] ~~`install_csharp_ls` 的 stderr 未重定向~~ **已修复**
- **文件:** `scripts/install_claude_code.sh:199`
- **维度:** 代码质量
- **描述:** `dotnet tool install -g csharp-ls >/dev/null` 只重定向 stdout，stderr 错误直接打印到终端。
- **修复建议:** 改为 `&>/dev/null`。
- **修复:** `>/dev/null` → `&>/dev/null`。

---

#### [S-019] ~~`install_pixi.sh` 使用防御性 source，其他脚本不使用，风格不一致~~ **已修复**
...
- **修复:** 改为直接 `source "$SCRIPT_DIR/../lib/utils.sh"`，与其他脚本统一。

---

#### [S-020] ~~`INSTALLED_PLUGINS_JSON` 变量定义但从未使用~~ **已修复**
...
#### [S-022] ~~`setup_claude_hud` 使用 `eval` 执行动态命令~~ **已修复**
- **文件:** `scripts/install_claude_code.sh:745`
- **维度:** 安全性
- **描述:** `eval "$hud_cmd"` 中 `$hud_cmd` 由脚本控制非用户输入，但 `eval` 是安全审查高风险标记。
- **修复建议:** 添加注释说明内容完全由脚本控制，无注入风险。
- **修复:** 在 `eval` 行上方添加安全注释说明 `$hud_cmd` 由 `_detect_hud_runtime()` 构建，无注入风险。

---

#### [S-026] ~~多个 LSP 安装函数的平台检测重复~~ **已修复**
...
- **修复:** 在 `lib/utils.sh` 添加 `get_platform_triple()`（Rust triple 格式），rust-analyzer 的 14 行 if-else 缩减为 1 行调用。lua-ls 格式不同，简化为一行条件赋值并添加注释。

---

#### [S-028] ~~brew tap 检查模式重复~~ **已修复**
- **文件:** `scripts/macos_install.sh:89,109`
- **维度:** 代码精简度
- **描述:** 两次相同的 `brew tap | grep -q ... || brew tap ...` 模式。
- **修复建议:** 提取 `ensure_brew_tap()` 函数。
- **修复:** 提取 `ensure_brew_tap()` 函数，两处调用统一为 `ensure_brew_tap "tap名"`。

---

#### [S-030] ~~`ensure_study_master_hooks` 的 jq 表达式嵌套复杂~~ **已修复**
- **文件:** `scripts/install_claude_code.sh:606-614`
- **维度:** 代码精简度
- **描述:** 嵌套 jq（条件判断+数组追加+默认值）在 shell 中难调试。
- **修复建议:** 可接受，建议添加行内注释或移到 `.jq` 文件。
- **修复:** jq 逻辑已是最简形式（"条件追加嵌套数组"本身复杂），添加了行内注释说明每步意图。

---

#### [S-032] ~~npm LSP 安装代码重复，可用循环替代~~ **已修复**
- **文件:** `scripts/install_claude_code.sh:168-190`
- **维度:** 代码精简度
- **描述:** `typescript-language-server` 和 `intelephense` 安装逻辑完全相同只是包名不同。
- **修复建议:** 用数组+循环替代。
- **修复:** 用 `npm_lsps` 数组（`"命令名:安装包"` 格式）+ 循环替代，新增 LSP 只需加一行数组元素。

---

#### [S-033] ~~`copy_path` 每个文件都打印成功消息，噪音较多~~ **已修复**
- **文件:** `scripts/install_dotfiles.sh:36`
- **维度:** 代码精简度
- **描述:** 部署 20+ 文件时产生大量输出，淹没重要信息。
- **修复建议:** 改用 `print_dim`，全部完成后用 `print_success` 汇总。
- **修复:** `copy_path` 改为 `print_dim`，main 末尾添加 `print_success "Dotfiles 配置部署完成"` 汇总。

---

#### [S-034] ~~多个 LSP 安装函数各自判断 "Linux only"~~ **已修复**
...
- **修复:** 3 个函数删除各自的 `if macos return` 检查，main() 中统一用 `if [[ "$OS" != "macos" ]]` 包裹调用。

---

#### [S-036] ~~`macos_install.sh` 不包装到 main() 函数~~ **已修复**
- **文件:** `scripts/macos_install.sh`
- **维度:** 代码质量
- **描述:** 所有代码在顶层执行，不检查参数，与其他脚本风格不一致。
- **修复建议:** 包装到 `main()` 中。
- **修复:** 所有逻辑包装到 `main()` 中，末尾 `main "$@"` 调用。
...
#### [Z-022] ~~`typeset -U path` 位置偏晚~~ **已修复**
- **文件:** `.zshrc:46`
- **维度:** 代码质量
- **描述:** 设在 `.zshrc` 但 `.zprofile` 中 brew shellenv 可能已插入重复条目。
- **修复建议:** 移到 `.zshenv` 中确保整个加载链去重。
- **修复:** 移到 `.zshenv` 中（`typeset -U path fpath`），`.zshrc` 原位置改为注释说明。

---

#### [Z-023] ~~`fzf-preview.sh` 中 bat 检测逻辑重复~~ **已修复**
- **文件:** `.config/zsh/fzf/fzf-preview.sh:21-28,42-49`
- **维度:** 代码精简度
- **描述:** `batcat`/`bat` 检测完全相同出现两次。
- **修复建议:** 提取到文件开头一次检测，后续引用变量。
- **修复:** 提取到文件开头一次检测赋值 `batname`，两处引用 `$batname`（空则回退 echo/cat）。

---

#### [Z-025] ~~`zinit.zsh` 中 eza 别名连续两次 `$+commands` 检查~~ **已修复**
...
#### [K-006] ~~`smart_tab.py` 和 `smart_window.py` 的 `handle_result` 高度相似~~ **已修复**
...
- **修复:** 见 K-005，已提取 `smart_launch()` 到 `ssh_utils.py`。

---

#### [K-007] ~~Catppuccin 主题文件已注释但仍保留在仓库~~ **已修复**
- **文件:** `.config/kitty/kitty.conf:238`
- **维度:** 架构健康度
- **描述:** `include ./Catppuccin-Frappe.conf` 已注释，文件仍存在。
- **修复建议:** 如不再使用则移除文件，如临时切换则添加注释说明。
- **修复:** 保留文件备用切换，在注释中说明原因。

---

#### [K-011] ~~注释拼写错误 `siwtch windwos`~~ **已修复**
...
#### [H-005] ~~`AppToggler` 可用原生 API 简化~~ **已修复**
...
- **修复:** 见 H-004，已简化为原生 API，4 行核心逻辑。

---

#### [H-012] ~~`systemInfo.lua` 中大量重复的 `hs.styledtext.new` 样式参数~~ **已修复**
- **文件:** `.hammerspoon/modules/systemInfo.lua:237-252`
- **维度:** 代码精简度
- **描述:** 四处相同样式 table 每次内联创建。
- **修复建议:** 提取为 `local STYLE = {...}` 常量复用。
- **修复:** 提取为 `local MENUBAR_STYLE = {...}` 常量，四处 `hs.styledtext.new` 统一引用。

---

#### [H-015] ~~`init.lua` 和 `reload.lua` 双重发送启动通知~~ **已修复**
- **文件:** `.hammerspoon/init.lua:22` 和 `.hammerspoon/modules/reload.lua:20`
- **维度:** 代码质量
- **描述:** 每次启动弹两条通知，冗余。
- **修复建议:** 删除 `reload.lua` 的通知，只保留 `init.lua` 末尾的。
- **修复:** 见 H-016/H-017，reload.lua 重构时已删除通知。

---

#### [H-019] ~~`inputMethod.lua` 模块已废弃但仍被 require~~ **已修复**
...
- **修复:** 见 H-023/H-024，init.lua 重构时已移除 `require('modules.inputMethod')`。

---

#### [H-025] ~~`systemInfo.lua` 中 35+ 行注释掉的代码~~ **已修复**
- **文件:** `.hammerspoon/modules/systemInfo.lua:195-216,315-327`
- **维度:** 代码精简度
- **描述:** 占文件 10%，增加阅读负担。Git 历史已保存。
- **修复建议:** 删除注释掉的废弃代码。
- **修复:** 删除菜单快捷键（21 行）和底部函数（13 行）两段注释掉的废弃代码。

---

#### [H-026] ~~`formatPercent` 对 `percent <= 0` 返回 `"  1%"` 而非 `"  0%"`~~ **已修复**
...
#### [H-028] ~~`base.lua` 中 UTF-8 函数和双端队列函数均未使用~~ **已修复**
...
- **修复建议:** 删除未使用的函数。
- **修复:** 见 H-002，base.lua 重构时已删除全部未使用函数（165 行 → 31 行）。

---

#### [E-003] ~~Code vs Cursor markdown 预览设置不一致~~ **已修复**
...
- **修复建议:** 统一为 `true`。
- **修复:** Cursor Library 版改为 `true`，sync hook 自动同步到 .config 版本。

---

#### [E-007] ~~keybindings.json 末尾 trailing comma（所有 4 个版本）~~ **已修复**
- **文件:** `.config/Code/User/keybindings.json:34-35` 等
- **维度:** 代码质量
- **描述:** JSONC 容忍但标准 JSON 不合法，其他工具可能解析失败。
- **修复建议:** 删除最后一个 `}` 后的逗号。
- **修复:** 4 个版本全部删除 trailing comma。

---

#### [C-003] ~~.gitconfig URL rewrite 全局将 HTTPS 转 SSH~~ **已修复**
- **文件:** `.gitconfig:5-6`
- **维度:** 安全性
- **描述:** 无 SSH key 环境（CI、容器）中所有 GitHub 操作会失败。
- **修复建议:** 在注释中说明影响和临时绕过方法。
- **修复:** 添加注释说明 SSH 依赖和绕过命令 `git -c url."https://github.com/".insteadOf="" ...`。

---

#### [C-004] ~~direnv.toml 使用未替换的 `__HOME__` 占位符~~ **已修复**
- **文件:** `.config/direnv/direnv.toml:7`
- **维度:** 代码质量
- **描述:** 需安装脚本 sed 替换，直接复制部署时 direnv 白名单失效且无错误提示。
- **修复建议:** 添加醒目注释标注不能直接复制部署。
- **修复:** 添加 ⚠ 醒目注释说明不能直接 cp，必须通过 install_dotfiles.sh 部署。

---

#### [C-006] ~~.envrc PATH 去重管道可能影响含空格路径~~ **已修复**
- **文件:** `.envrc:8`
- **维度:** 代码质量
- **描述:** `tr ':' '\n'` 管道方式不够健壮（虽然 PATH 极少含空格）。
- **修复建议:** 可用 `typeset -U PATH` (zsh) 或 `awk -v RS=:` 替代。
- **修复:** 改为 `awk -v RS=: -v ORS=:`，直接用冒号作分隔符，不经过换行符中间步骤。

---

#### [C-007] ~~ripgrep config 默认 `--ignore-case`~~ **已修复**
- **文件:** `.config/ripgrep/config:2`
- **维度:** 代码质量
- **描述:** 代码搜索中大小写不敏感可能产生噪音。
- **修复建议:** 考虑改用 `--smart-case`。
- **修复:** `--ignore-case` → `--smart-case`（搜索全小写时不敏感，包含大写时精确匹配）。

---

#### [T-004] ~~get-my-ip.sh 国旗 case 语句有 4 个重复条目~~ **已修复**
- **文件:** `sh-script/get-my-ip.sh:40/64, 48/75, 72/76, 53/77`
- **维度:** 代码精简度
- **描述:** SE、TH、CL、NG 各出现两次，后面的是死代码。
- **修复建议:** 删除重复条目。
- **修复:** 删除 SE、TH、CL、NG 四个重复 case 条目。

---

#### [T-005] ~~get-my-ip.sh 缺少 `set -eo pipefail`~~ **已修复**
- **文件:** `sh-script/get-my-ip.sh:1`
- **维度:** 代码质量
- **描述:** 未覆盖的错误路径会被静默忽略。
- **修复建议:** 添加 `set -eo pipefail`。
- **修复:** 添加 `set -eo pipefail`。

---

#### [T-007] ~~get-my-ip.sh 依赖 jq 但未检查~~ **已修复**
- **文件:** `sh-script/get-my-ip.sh:18`
- **维度:** 代码质量
- **描述:** jq 不存在时报错不友好。
- **修复建议:** 开头添加 `command -v jq &>/dev/null || { echo "Error: jq required"; exit 1; }`。
- **修复:** 在 `set -eo pipefail` 后添加 jq 依赖检查。

---

#### [D-003] ~~check-file-deps.sh 路径前缀剥离依赖字面匹配~~ **已修复**
- **文件:** `.claude/hooks/check-file-deps.sh:21-25`
- **维度:** 代码质量
- **描述:** 符号链接解析后的路径会导致前缀匹配失败，hook 静默退出。
- **修复建议:** 用 `realpath` 规范化路径后再匹配。
- **修复:** `PROJECT_ROOT` 用 `pwd -P` 解析符号链接，`file_path` 用 `realpath` 规范化。

---

#### [D-005] ~~file-deps.json 缺少 .envrc 和 direnv.toml 的依赖~~ **已修复**
- **文件:** `.claude/file-deps.json`
- **维度:** 架构健康度
- **描述:** 修改 `.envrc` 不会提醒检查 `pixi.toml` 和模板文件。
- **修复建议:** 添加关联映射。
- **修复:** 添加 `.envrc` → `pixi.toml` + `envrc-project-template.sh`，`.config/direnv/direnv.toml` → `install_dotfiles.sh` 的依赖。
...
|------|----------|---------|------|------|
| lib/ 基础库 | 0 | 7 | 10 | 17 |
| install/uninstall | 1 | 11 | 12 | 24 |
| scripts/ 安装脚本 | 2 | 12 | 19 | 33 |
| Shell 配置 | 1 | 14 | 7 | 22 |
| Kitty | 2 | 5 | 4 | 11 |
| Hammerspoon | 1 | 15 | 7 | 23 |
| 编辑器配置 | 0 | 5 | 2 | 7 |
| 其他配置 | 1 | 2 | 4 | 7 |
| 独立脚本 | 0 | 4 | 3 | 7 |
| Claude Code | 0 | 3 | 2 | 5 |
...
> 注：部分问题涉及多个维度，维度合计（122）小于问题总数（156）是因为每个问题只计入主维度。Warning 中有 19 个问题为次要维度重叠（如安全性+代码精简度），在此表中按主维度计。
